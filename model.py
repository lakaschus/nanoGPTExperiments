...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att = torch.einsum('bnij,bnkj -> bnik', q, k) * (1.0 / math.sqrt(k.size(-1))) # (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)
        y = torch.einsum('bnij,bnjl->bnil', att, v) # att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
